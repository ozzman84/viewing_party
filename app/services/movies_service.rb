class MoviesService < ApiService
  def self.top_40_movies
    ...
    page2 = receive_data('https://api.themoviedb.org/3/discover/movie?sort_by=vote_count.desc&api_key=da336b32ae4779ba9aa007085c1574ec')
    movie_list_maker(page1) + movie_list_maker(page2)
  end

  def self.movies_by_name(search_string)
    page1 = receive_data("https://api.themoviedb.org/3/search/keyword?api_key=da336b32ae4779ba9aa007085c1574ec&query=#{search_string}")
    if page1[:total_pages] > 1
      page2 = receive_data("https://api.themoviedb.org/3/search/keyword?api_key=da336b32ae4779ba9aa007085c1574ec&query=#{search_string}&page=2")
      movie_list_maker(page1) + movie_list_maker(page2)
    else
  ...
  def self.movie_maker(movie_id)
    ...
    movie = {id: movie_details[:id], original_title: movie_details[:original_title], vote_average: movie_details[:vote_average], runtime: movie_details[:runtime], genres: movie_details[:genres], overview: movie_details[:overview], cast: cast[:cast], reviews: reviews[:results]}
  end
end
